...
OBSScene GetCurrentScene1()
{
    obs_scene_t* outputScene = 0;
    obs_source_t* outputSource = obs_get_output_source(0);////  获取输出源
    if (outputSource)
    {
        outputScene = obs_scene_from_source(outputSource);//从输出源获取场景
...
//        qDebug()<<"point"<<point.y;
        ...
        if(point.y <250){
            ytop = 0;
            ybottom = 1080 -500;
        }else if(point.y >1080 -250){
            ybottom = 0;
            ytop = 1080 -500;
        } else{
            ytop = point.y - 250;
            ybottom = 1080 - (point.y +250);
        }
        //1920,
        if(point.x <250){
            xleft = 0;
            xright = 1920 -500;
        }else if(point.x >1920 -250){
            xright = 0;
            xleft = 1920 -500;
        } else{
            xleft = point.x - 250;
            xright = 1920 - (point.x +250);
        }

        ////////////////
        obs_sceneitem_crop crop;
        crop.left   = uint32_t(xleft);
        crop.right  = uint32_t(xright);
        crop.top    = uint32_t(ytop);
        crop.bottom = uint32_t(ybottom);
