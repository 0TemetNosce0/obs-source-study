...
{
    D3D11_TEXTURE2D_DESC desc;//描述了一个2d纹理
    tex->GetDesc(&desc); //获取2d纹理
    ...
    if (!!d->texture)
        d->device->context->CopyResource(d->texture->texture,
                tex);//tex复制到texture。将tex源资源的整个内容复制到GPU使用的目标资源texture。
...
{
    DXGI_OUTDUPL_FRAME_INFO info;//DXGI_OUTDUPL_FRAME_INFO结构描述当前的桌面映像。
    ComPtr<ID3D11Texture2D> tex;// 2d纹理
    ComPtr<IDXGIResource> res;//
    HRESULT hr;

    hr = d->duplicator->AcquireNextFrame(0, &info, res.Assign());//指示应用程序已准备好处理下一个桌面映像,res.Assign()它接收包含桌面位图的表面的IDXGIResource接口。
    ...
    hr = res->QueryInterface(__uuidof(ID3D11Texture2D),
            (void**)tex.Assign());//QueryInterface客户可以通过此函数来查询某个组件是否支持某个特定的接口.COM
    if (FAILED(hr)) {
