...
{
    return _InterlockedDecrement(val);//都是对变量加一或减一
}
...
{
    return _InterlockedCompareExchange(val, new_val, old_val) == old_val;//InterlockedCompareExchange是把目标操作数（第1参数所指向的内存中的数）与一个值（第3参数）比较，如果相等，则用另一个值（第2参数）与目标操作数（第1参数所指向的内存中的数）交换
}
